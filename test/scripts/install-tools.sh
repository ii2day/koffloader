...
if ! cilium version --client &>/dev/null ; then
    echo "fail 'cilium-cli' miss"
    if [ -z "$JUST_CLI_CHECK" ] ; then
      echo "try to install it"
      CLI_ARCH=amd64
      if [ -z $http_proxy ]; then
        CILIUM_CLI_VERSION=$(curl -s https://raw.githubusercontent.com/cilium/cilium-cli/main/stable.txt)
        curl -Lo /tmp/cilium-linux-${CLI_ARCH}.tar.gz https://github.com/cilium/cilium-cli/releases/download/${CILIUM_CLI_VERSION}/cilium-linux-${CLI_ARCH}.tar.gz
      else
        CILIUM_CLI_VERSION=$(curl -s https://raw.githubusercontent.com/cilium/cilium-cli/main/stable.txt)
        curl -x $http_proxy -Lo /tmp/cilium-linux-${CLI_ARCH}.tar.gz https://github.com/cilium/cilium-cli/releases/download/${CILIUM_CLI_VERSION}/cilium-linux-${CLI_ARCH}.tar.gz
      fi
      tar -C /usr/local/bin -xzf /tmp/cilium-linux-${CLI_ARCH}.tar.gz
      chmod +x /usr/local/bin/cilium
      ! cilium version --client &>/dev/null && echo "error, failed to install cilium-cli" && exit 1
    fi
    MISS_FLAG="true"
else
    echo "pass   'cilium-cli' installed:  $( cilium version --client 2>&1 ) "
fi
